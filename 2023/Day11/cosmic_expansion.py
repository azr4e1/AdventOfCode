...
if __name__ == "__main__":
    ...
    with open("./input.txt") as f:
        for line in f:
            universe_str.append(line.strip())
    universe = Universe(universe_str)
    universe.expand()
    distances = universe.calculate_distances()
    total_distances = sum(distances.values())

    print("Sum of the distances is:", total_distances)

    infinite_universe = Universe(universe_str)
    infinite_universe.expand(1_000_000)
    infinite_distances = infinite_universe.calculate_distances()
    infinite_total_distances = sum(infinite_distances.values())

    print("Sum of the distances with 1,000,000 expansion factor:", infinite_total_distances)
